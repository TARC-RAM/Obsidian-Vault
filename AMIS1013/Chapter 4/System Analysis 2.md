# Fact Recording
- Purpose - Facts about the existing system must be recorded / (documented) so that they can be referred to during the subsequent stages.
...
![[Pasted image 20260225162020.png]]
# Data Flow Diagram
![[Pasted image 20260225145150.png]]
# Definition of DFD
- Graphical Models - DFD uses graphical model to document the operation of the current system.
- (Processes) - The processes of the current system are modeled with a set of symbols or notations.
- (Notation) - The diagramming notation used: process, data store, data flow and entity.
...
# Data Flow
- Definition
	- A (path) by which data moves from one part of the IS to another part.
...
# Process
- Definition
	- Describe the main (transformations) that take place within the system. It changes incoming data flows into outgoing data flows
- Examples
	- Calculate gross pay
...
- Definition
	...
	- It is a file or (table).
- Examples
	- Student Scores
	- Employee Salary and Deduction data
- Symbol/Notation
	- An open-ended rectangle
	- Labeled with a name (noun) and a number as an identifier
# Entities
- Definition
...
- Definition - represents the entire software element as a single process (circle) with input and output data indicated by incoming and outgoing arrow. Boundary indicated by external entities.
...
# Some Common Errors
## Incorrect Combination of Date Flow and Process Symbols
![[Pasted image 20260225160607.png]]
# Incorrect Combination of Data Flow and Process Symbols
...
![[Pasted image 20260225160710.png]]
# Example of Common Errors
![[Pasted image 20260225160730.png]]
# Correct Combination of Data Flow and Process Symbols
![[Pasted image 20260225160755.png]]
# Incorrect Uses of (Process) Symbols
![[Pasted image 20260225160821.png]]
# (Incorrect) Use of Data Store Symbols
![[Pasted image 20260225160846.png]]
# () Between Systems
![[Pasted image 20260225160856.png]]
# Example 1 - Order Processing
- Orders received from customers are checked that the product and payment-type are valid values, and there is sufficient stock.
- Accepted order details are stored on an order file. If not, the rejected orders are sent back to the customer and the whole order is put on hold and an explanatory letter is sent to the customer.
- Goods are received from suppliers by referring to the PO and GRN is generated. Stock is updated
- The 'on-hold' orders are checked and converted into an ordinary order when there is sufficient stock.
- Despatch notes are raised for all orders received. A copy of the dispatch note is sent to the customer and a second copy is sent to the warehouse. The dispatch-date is noted on the order file.
- At the end of the week an invoice is raised and sent to the customer. The invoice-date is noted on the order file.